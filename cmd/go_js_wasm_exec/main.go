...
func handleWorker(ctx context.Context, info *target.Info, logger *log.Logger) {
	if info.Type != "worker" {
		return
	}
	wctx, cancel := chromedp.NewContext(ctx, chromedp.WithTargetID(info.TargetID))
	defer cancel()
	chromedp.ListenTarget(wctx, func(ev interface{}) {
		handleEvent(wctx, ev, logger)
	})
	chromedp.Run(wctx)
	<-ctx.Done()
}
...
func handleEvent(ctx context.Context, ev interface{}, logger *log.Logger) {
	...
	case *target.EventAttachedToTarget:
		go handleWorker(ctx, ev.TargetInfo, logger)
